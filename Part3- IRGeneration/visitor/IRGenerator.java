 ...
 */
public class IRGenerator<A> implements GJVisitor<String, A> {
   boolean debug = false;
   ...
   String messageSendReturnType = null;
   ...
   void generateSymbolTables() {
    for(String className : classes.keySet()) {
        ...
        while(currentClass.isChild) {
            ...
            for(_variable currField : currentClass.fields) {
                _variable newField = new _variable();
                newField.type = currentClass.name;
                newField.name = currField.name;
                currSymbolTable.fields.add(newField);
            }
        }

        Collections.reverse(currSymbolTable.fields);

        currentClass = classes.get(className);
        Stack<String> parent = new Stack<String>();
        parent.push(currentClass.name);
        while(currentClass.isChild) {
            currentClass = classes.get(currentClass.parentClass);
            parent.push(currentClass.name);
        }
        ...
        while(!parent.empty()) {
            String newClass = parent.pop();

            for(_method currMethod : classes.get(newClass).methods) {
                boolean overridden = false;
                for(_method method : currSymbolTable.methods) {
                    if(method.name.equals(currMethod.name)) {
                        overridden = true;
                        break;
                    }
                }
                if(!overridden) {
                    ...
                    newMethod.enclosingClass = newClass;
                    ...
                    for(_method method : currSymbolTable.methods) {
                        if(method.name.equals(currMethod.name)) {
                            method.enclosingClass = newClass;
                            break;
                        }
                    }
                }
   ...
   String generateNewObject(String className) {
      ...
      if(currentSymbolTable.methods.size() > 0) {
         ...
         for(int i=0; i<currentSymbolTable.methods.size(); i++) {
            print("MOVE TEMP " + tempCount + " " + currentSymbolTable.methods.get(i).enclosingClass + "__" + currentSymbolTable.methods.get(i).name);
   ...
   String findFieldIndex(String variableType, String fieldName) {
      ...
      for(int i=0; i<currentSymbolTable.fields.size(); i++) {
         ...
         if(currentSymbolTable.fields.get(i).name.equals(fieldName) && currentSymbolTable.fields.get(i).type.equals(variableType)) {
            return String.valueOf((i + 1) * 4);
         }
      }

      if(lastRef != -1)
        return String.valueOf((lastRef + 1) * 4);
      return null;
   }
   ...
   public String visit(MethodDeclaration n, A argu) {
      ...
      if(!generateIR) {
         ...
         print(currentClassName + "__" + methodName + " [" + (currentMethod.parameters.size() + 1) + "]");
         print("BEGIN");

         tempCount = currentMethod.parameters.size() + currentMethod.variables.size() + 1;

         n.f8.accept(this, (A) currentMethod);

         String returnTemp = n.f10.accept(this, (A) currentMethod);

         if(returnTemp.equals("this")) returnTemp = "0";
   ...
   public String visit(ArrayAssignmentStatement n, A argu) {
      ...
      String indexTemp = n.f2.accept(this, argu);

      String valueTemp = n.f5.accept(this, argu);

      print("MOVE TEMP " + (tempCount) + " PLUS TEMP " + indexTemp + " 1");
      String effectiveIndexTemp = String.valueOf(tempCount++);
      print("MOVE TEMP " + (tempCount) + " TIMES TEMP " + effectiveIndexTemp + " 4");
      String offsetTemp = String.valueOf(tempCount++);
   ...
   public String visit(OrExpression n, A argu) {
      ...
      String endLabel = getLabel();
      String tempLabel1 = getLabel();
      String tempLabel2 = getLabel();
   ...
   public String visit(neqExpression n, A argu) {
      ...
      if(leftTemp.equals("this")) leftTemp = "0";
      if(rightTemp.equals("this")) rightTemp = "0";
   ...
   public String visit(ArrayLookup n, A argu) {
      ...
      print("MOVE TEMP " + (tempCount) + " PLUS TEMP " + index + " 1");
      String effectiveIndex = String.valueOf(tempCount++);
      print("MOVE TEMP " + (tempCount) + " TIMES TEMP " + effectiveIndex + " 4");
      String offset = String.valueOf(tempCount++);
   ...
   public String visit(MessageSend n, A argu) {
      ...
      if(variableName.matches("[0-9]+")){
         variableType = messageSendReturnType;
         ...
         if(variableTemp == null) {
            variableTemp = getFieldTemp(((_method) argu).enclosingClass, variableName);
        }
      }
      ...
      messageSendReturnType = methodIndex_ReturnType.split("_")[1];
      ...
      for(int i = 0; i < arguments.size(); i++) {
         if(arguments.get(i).equals("this")) System.out.print("TEMP 0 ");
         else System.out.print("TEMP " + arguments.get(i) + " ");
      }
      print(")");
   ...
   public String visit(Identifier n, A argu) {
      ...
      if(isMessageSend) return name;
         ...
         _ret = getTemp((_method) argu, name);
         if(_ret == null) {
